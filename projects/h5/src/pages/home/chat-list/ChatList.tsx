...
import { NavBar, Avatar, Space, List, SwipeAction, Dialog } from 'antd-mobile';
...
export const ChatList: FC<IChatListProps> = memo(({}) => {
  ...
  const right = (
    <Space style={{ fontSize: 24, '--gap': '16px' }}>
      <EvoIcon size={'small'} onClick={handleSearch} style={{color: 'var(--adm-color-text)'}} type="icon-search" />
      <EvoIcon size={'small'} onClick={handleNewChat} style={{color: 'var(--adm-color-text)'}} type="icon-message" />
    </Space>
  )

  return (
    <Space direction='vertical' style={{ width: '100%', height: '100%' }}>
      <NavBar right={right} back={null} left={<Avatar src='' style={{ '--size': '32px', '--border-radius': '16px' }}/>} />
      {/* <ChatListHeader

...
export const composeMcpTools = async (params: {
  mcpIds: IChatWindowConfig['mcpIds'];
}): Promise<TComposedContexts> => {
  const { mcpIds } = params;

  const result: TComposedContexts = [];

  await Promise.all(
    mcpIds?.map(async (id) => {
      const res = await McpBridgeFactory.getInstance().getTools(id);

      if (res.success && res.data) {
        result.push({
          role: 'system',
          content: getSystemPrompt('', res.data as any),
        });
      }
